...
document.body.addEventListener('mouseup', e => {
	...
	var url = "https://www.wordreference.com/enit/" + text;
    var wr = window.open(url, '_blank');
	wr.blur();
	window.focus();
});
...
var formatTweetDeckSections = function formatTweetDeckSections() {
	...
	setTimeout(function () {
		var nbaTitle = sections[1].querySelector("span.column-heading");
		if (nbaTitle != null) {
			nbaTitle.innerText = "NBA";
		}
...
var openNewImage = function openNewImage() {
	...
	console.log(chrome.runtime.id, extensionId)
