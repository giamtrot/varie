...
function forwardOpenTab(event: MessageEvent<any>) {
    ...
    chrome.runtime.sendMessage(event.data, function (response: any) {
        console.log(response);
    });
}

function loadScripts(files: string[], world: string) {
    chrome.runtime.sendMessage({ action: "loadScripts", files: files, world: world }, function (response: any) {
        console.log(response)
    });
}
