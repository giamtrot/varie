const EXT_ID_RL = "RL - 2025.01.28-1"
...
function loadUI() {
	...
	const baseObserver = new MutationObserver(() => {
		const targetNode = Array.from(document.querySelectorAll('span'))
			.find(el => el.textContent?.trim() === 'Visualizza ricetta');
...
function addRLUI(targetNode: Element) {
	log("addRLUI")
	...
	var button = document.createElement("INPUT") as HTMLInputElement;
	button.type = "button"
...
async function searchAll() {
	...
	const select = document.querySelector("select#provincia") as HTMLSelectElement
	const options = select?.options
	for (const option of Array.from(options)) {
		console.log(option);

		await fetchProvincia(option.value, option.label)
		return
	}
}

async function fetchProvincia(desc: String, codice: String) {

	const codiceProvincia = codice.substring(0, 3);
	const codiceComune = codice.substring(4);
	log("fetchProvincia", desc, codice, codiceProvincia, codiceComune)
	const jsonBusinessArg0 =
	{
	...
	const encodedJsonBusinessArg0 = encodeURIComponent(JSON.stringify(jsonBusinessArg0));
	const body = "metadata={\"destination\":\"pgpcitt_ricerca_disponibilita\",\"method\":\"handle\",\"argumentTypes\":[\"java.lang.String\"]}&&jsonBusinessArg0=" + encodedJsonBusinessArg0
	log("body", body)

	const ris = await fetch("https://www.fascicolosanitario.regione.lombardia.it/prenotaonline/jsonBroker", {
		// "headers": {
