...
"ildisperso", "ilgiocodeiregni", "ilgiornodelgiudizio", "ilgiuocodelleperledivetro", "ilgrandebob", "ilmaestroemargherita", 
"iquarantagiornidelmussadagh", "ilmantello", "ilmonaco", "ilnaso", "ilpaesedicuccagna", "ilpiacere", "ilpranzodibabette", 
...
TO_INCLUDE.forEach{ href -> {
    if (!done) {
        ...
        if (!file.exists()) {
            ...
            allItems.addAll(items)
            makeRSSFromItems(file, bookTitle, items, false)
            done = true
        }
    }
}}
...
makeRSSFromItems(adAltaVoceRSS, "Ad Alta Voce", allItems, true)
...
def makeRSSFromItems(file, titleValue, items, global) {
    ...
    xmlMarkup.rss {
        channel {
            ...
            if (global) {
                image {
                      url("https://www.raiplaysound.it/resizegd/252x252/dl/img/2021/11/29/1638210894856_2048x2048%20ad%20alta%20voce%202.jpg")
                    title("Ad Alta Voce")
                      link("https://www.raiplaysound.it/programmi/adaltavoce/audiolibri")
                }
            }
            items.each{ i ->
                'item' {
